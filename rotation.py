...
while True:
    ...
    if hands:
        ...
        if circle_bound:
            if landmark[0] > rightShoulder[0] and landmark[1] < (((landmark[0]-s[0])/(s[0]-e[0])) * (s[1]-e[1]) + s[1]) and landmark[1] > (((landmark[0]-c[0])/(c[0]-d[0])) * (c[1]-d[1]) + c[1]):
                print("Right side")
        ...
        if circle_bound:
            if landmark[1] < rightShoulder[1] and landmark[0] > (((landmark[1]-s[1])/(s[1]-e[1])) * (s[0]-e[0]) + s[0]) and landmark[0] < (((landmark[1]-c[1])/(c[1]-d[1])) * (c[0]-d[0]) + c[0]):
                print("Up side")
